...
async function hReg(values){
  ...
  setIsLoading(true)
  try
  {const reg= await axios.post('https://ecommerce.routemisr.com/api/v1/auth/signup',values);
  console.log(reg);
...
function validateForm(values)
{
  ...
  {
    errors.name="Must be more than 3 ch and less than 28"
  }
  ...
  {
    errors.rePassword="Confirm Passwrod Is Required"
  }
  
  else if(!values.password==values.rePassword)
    {errors.rePassword="Confirm Password should be like Password "}
  else{
    errors.rePassword=""
  }
...
<form  className="lg:max-w-2xl mx-auto max-w-sm  " onSubmit={formik.handleSubmit}>
  ...
  <div  className="relative z-0 w-full mb-5 group">
      <input onBlur={formik.handleBlur} type="password" name="rePassword" id="rePassword"  className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required onChange={formik.handleChange} value={formik.values.rePassword}  />
      <label for="floating_repeat_password"  className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6" >Confirm password</label>
  </div>
      {formik.errors.rePassword&& formik.touched.rePassword &&(<div className='bg-red-300 border-red-900 border-2 text-red-900 rounded  m-3 mb-5 p-3'>{formik.errors.rePassword}</div>)}

    <div  className="relative z-0 w-full mb-5 group">
        <input onBlur={formik.handleBlur} type="text" name="name" id="name"  className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required onChange={formik.handleChange} value={formik.values.name}  />
        <label for="floating_first_name"  className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6" onChange={formik.handleChange} value={formik.values.name} >Full name</label>
    </div>
        {formik.errors.name&& formik.touched.name &&(<div className='bg-red-300 border-red-900 border-2 text-red-900 rounded  m-3 mb-5 p-3'>{formik.errors.name}</div>)}
